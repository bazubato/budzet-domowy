...
Aplikacja ma już wbudowaną obsługę „Dodaj do ekranu głównego" — po
wdrożeniu (patrz sekcja niżej) będzie miała własną ikonę i otwierać się
na pełnym ekranie, bez paska przeglądarki.

1. Najpierw wdróż aplikację (najszybciej: Vercel lub Netlify, patrz niżej) —
   potrzebujesz gotowego adresu w internecie (np. `budzet-domowy.vercel.app`).
   Samo `npm run dev` na komputerze nie wystarczy, żeby zainstalować to na
   telefonie.
2. Otwórz ten adres w przeglądarce na telefonie.
3. **iPhone (Safari):** stuknij ikonę „Udostępnij" (kwadrat ze strzałką) na
   dole ekranu → „Dodaj do ekranu początkowego".
4. **Android (Chrome):** stuknij trzy kropki w prawym górnym rogu →
   „Dodaj do ekranu głównego" (albo Chrome sam zaproponuje to jako baner
   „Zainstaluj aplikację").

Od tej pory ikona aplikacji będzie na ekranie głównym telefonu jak każda
inna appka.
...
```
├── index.html          punkt wejścia HTML (z manifestem PWA)
├── public/
│   ├── manifest.json     opis aplikacji dla „Dodaj do ekranu głównego"
